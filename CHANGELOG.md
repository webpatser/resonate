...
- **One client that stopped reading stalled the whole channel.** `Channel::broadcast()` walks its subscribers and writes to each in turn, and every write ends in the transport awaiting the socket: `WebsocketClient::sendText()` returns only once the bytes are accepted, and `Rfc6455Client` serialises anything behind it on `lastWrite`. So a subscriber whose TCP receive window went to zero suspended the fiber doing the fan-out. Every subscriber later in the loop waited on a peer with no intention of reading, and so did whatever issued the broadcast: an HTTP API event, a client event, a pub/sub envelope. Opening a socket and never reading from it was a channel-wide denial of service for the price of one connection.

  Each connection now owns a bounded FIFO drained by exactly one writer fiber (`Concurrency\SerialQueue`, held by `Server\RawConnection`). `send()` queues and returns, so the fan-out no longer waits on any peer, while the single writer keeps that connection's frames in exactly the order they were queued. Ordering is why the obvious fix, wrapping each send in `async()`, is the wrong one: concurrent fibers writing to one socket interleave, and a client that receives `member_removed` before the matching `member_added` is a worse bug than the stall. Sitting at the transport rather than in `Channel` is what makes one queue cover every write path: channel broadcasts, `pusher:*` replies from `EventHandler`, error frames from `Server::error()`, pings, and the close itself, which is queued behind the frames already waiting so a rejected connection still gets its error frame before the socket goes. A failed write is logged the way `Channel::sendTo()` logs one, never swallowed, and the writer fiber ends when its queue drains, so an idle or abruptly dropped connection holds no fiber.

  The bound is `servers.reverb.max_outbound_queue_size` (default `1000` messages). A connection that exceeds it is closed with WebSocket code `1013` (try again later) and its queue dropped. Buffering without a bound would only move the denial of service from time to memory, a client that cannot keep up has already lost the ordered stream it was subscribed to, and Pusher clients reconnect and resubscribe on their own. Set to `0` to disable the bound, which is only safe when every client is trusted.
- **One congested peer stalled all cross-node delivery.** `RedisPubSubProvider::subscribe()` handled every inbound envelope inline in the subscriber fiber, so anything an envelope did that suspended (writing to a stalled peer, gathering metrics from siblings, terminating a user's connections) was time in which nothing else arriving over Redis was delivered. Broadcasts, terminate requests and metrics replies from every other node queued behind it. Envelopes now go through the same bounded queue with a single worker, so the pump keeps draining the Redis subscription while handling stays strictly in arrival order, which is what the per-connection queues then preserve on the wire. The bound is `servers.reverb.scaling.max_queued_messages` (default `10_000` envelopes); an envelope arriving while the queue is full is dropped with a log line rather than buffered without limit. The resubscribe-with-backoff loop below is untouched.
...
- **One application's message size limit applied to every other application's connections.** `max_message_size` is configured per app, but the websocket parser takes its limit once, from the factory the client is built with, and `StartServer` resolved that single number as `max()` across all configured apps. The per-app check in `Pusher\Server::message()` is no defence: it runs on `$message->buffer()`, after the parser has assembled the whole message in memory. So a client of an app configured for 10KB could force the server to buffer up to the largest configured app's limit (10MB, say) before being rejected, then repeat. Two hundred such connections pinned gigabytes on a process serving every tenant, and the tenant paying for the small limit was the cheapest way in.

  The application is known during the handshake (the route is `/app/{appKey}` and the router's `Request` is handed to the client factory), so a new `ApplicationClientFactory` sizes each connection's parser from that connection's own application. An oversized frame is now refused on its header, with close code 1009, before a payload byte is buffered. The `Pusher\Server::message()` check stays as the protocol-level backstop.

  The global value survives only as the fallback for an upgrade whose app key resolves to nothing, and it is now the *smallest* configured limit rather than the largest: such a connection is closed with pusher code 4001 the moment the handler takes over, so the only thing the fallback governs is how much it can buffer in the meantime.

  Wire compatibility: a client of an app with a small `max_message_size` that was relying on a larger app's limit to send oversized messages now gets a 1009 close instead of a `pusher:error` frame. Every message within the app's own configured limit is unaffected.
...
- **Scaled channel metrics counted the requesting node twice.** Metrics envelopes carried no sender identity and there was no origin filter, and a node receives its own publications because the publisher and the subscriber are separate Redis connections. So the node that published a metrics request also received it, answered it as a responder, buffered its own reply, and then appended `local()` a second time. `mergeChannel()` sums `user_count` and `subscription_count`, so the CHANNEL and CHANNELS endpoints over-reported in every scaled deployment: two nodes with five users each reported fifteen. CHANNEL_USERS and CONNECTIONS came out right only by accident, through `unique()` and string-keyed merges respectively.

  Each node now stamps a stable per-process identity into every request envelope and drops any request carrying its own. The explicit local append stays rather than letting the node answer itself over Redis: local metrics are then gathered synchronously, are not at the mercy of a round-trip, and still appear if the pub/sub reply path is degraded. All four metric types now count each node exactly once.
- **Every scaled metrics request paid a fixed one second latency floor.** `gather()` suspended for the whole collection window regardless of how fast the siblings answered, so a `GET /apps/{id}/channels` cost a full second even when every reply had landed in 5ms, and a reply arriving after the window was discarded when the buffer was unset. Redis answers `PUBLISH` with the number of subscribers it delivered to, so the expected reply count is known with no extra round-trip and is scoped to exactly the envelope just sent (subtracting this node's own subscriber). The gather now completes the moment the last expected reply lands, and the window survives only as the upper bound for a node that never answers. Single-node-with-scaling-on returns immediately instead of waiting a second for nobody.

  The window is a plain referenced event-loop timer rather than a `TimeoutCancellation`, which unreferences its watcher: awaiting on one deadlocks whenever nothing else is holding the loop open. When it fires, the request resolves with whatever arrived rather than throwing.

  Tradeoff: under Redis Cluster the `PUBLISH` reply counts only the clients attached to the node that handled the command, so a clustered deployment can under-count and finish a gather before a sibling on another cluster node has answered. Regular pub/sub is not cluster-aware for counting purposes in general, and `PUBSUB NUMSUB` has the identical limitation; a node registry would avoid it at the cost of heartbeat writes, TTL tuning and its own staleness window. Point the scaling connection at a single Redis instance, which is what the documented configuration already describes.
...
- New configuration keys under `servers.reverb`: `max_channel_name_length` (`REVERB_MAX_CHANNEL_NAME_LENGTH`, default `255`), `max_subscriptions_per_connection` (`REVERB_MAX_SUBSCRIPTIONS_PER_CONNECTION`, default `250`) and `max_outbound_queue_size` (`REVERB_MAX_OUTBOUND_QUEUE_SIZE`, default `1000`), plus `scaling.max_queued_messages` (`REVERB_SCALING_MAX_QUEUED_MESSAGES`, default `10_000`). Published `config/reverb.php` files without them get the defaults; all four are documented in SECURITY.md.
- API change: `RawConnection::__construct()` takes the outbound bound as its second argument, `WebSocketHandler::__construct()` as its third, and `Factory::make()` as `$maxOutboundQueueSize`; `RedisPubSubProvider::__construct()` takes `$maxQueuedMessages` as its fourth. Every one is defaulted, so existing call sites keep working. `RawConnection::outbound()` and `RedisPubSubProvider::envelopes()` expose the queues for inspection.
...
- API change: `PubSubProvider::publish()` returns `int` (the number of subscribers the backend delivered the payload to, this node's own included) rather than `void`. `MetricsHandler` uses it to know how many sibling replies to expect. A custom provider that cannot report a count must return `0`, which degrades gathering to the full collection window rather than completing early on bad information.
- `MetricsHandler` takes the collection window as a second constructor argument (default `1.0` seconds, unchanged) and exposes `nodeId()`. `WebSocketHandler::appKey()` moved to a `ResolvesApplicationKey` trait, shared with the new `ApplicationClientFactory`.
