...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Install the Resonate dependencies';
    ...
    /**
     * Add the Resonate variables to the environment file.
     */
    ...
    /**
     * Publish the Resonate configuration file.
     */
    protected function publishConfiguration(): void
    {
        $this->callSilently('vendor:publish', [
            '--provider' => 'Webpatser\Resonate\ResonateServiceProvider',
            '--tag' => 'reverb-config',
        ]);
    }
    ...
    protected function updateBroadcastingConfiguration(): void
    {
        ...
        $path = app()->configPath('broadcasting.php');
     ...
     *
     * Laravel 10/11 apps ship a `providers` array in `config/app.php` with the
     * BroadcastServiceProvider commented out. Laravel 11+ skeleton apps register
     * providers through `bootstrap/providers.php` instead, and `config/app.php`
     * may not exist (or may not contain the provider line). Both cases are
     * supported here by no-oping when the file or the comment marker is absent.
     */
    protected function enableBroadcastServiceProvider(): void
    {
        $path = app()->configPath('app.php');
